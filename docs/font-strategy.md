...
> [!NOTE]
> 当初は「日本語=ヒラギノ明朝 ProN W3」採用だったが、commit `199ac95`（`chore: switch JP fallback fonts from Mincho to Hiragino Sans`）で Ghostty / Slack 等のフォールバックを **角ゴ (Hiragino Sans W3) に統一**した。Mincho 時代の議論 (BIZ UDMincho 比較含む) を読みたい場合は `git show 09eb250 -- docs/font-strategy.md` で当時の本文を参照可能。
...
| 日本語（ひらがな・カタカナ・漢字・全角記号） | **ヒラギノ角ゴシック ProN W3** (Hiragino Sans W3) |
...
エディタフォントは plist の `DVTSourceTextFont` キーで `JetBrainsMonoNF-Regular - 13.0` を指定。NF variant を選んでいるのは、Xcode のソース表示で Powerline 系 glyph を使わない（= Mono variant の利点が薄い）ため。
...
| **MarkdownObserver** | JetBrainsMono Nerd Font Mono | system-ui → ヒラギノ角ゴ ProN | CSS 変数 (`--c-mono` / `--c-sans`) | 設定済み (Claude Day テーマ) |
| **Xcode** | JetBrainsMono Nerd Font (Regular 13.0) | – (ソース表示中心) | xccolortheme `DVTSourceTextFont` | 設定済み (Claude Day テーマ) |
...
当初は「コードのゴシック感とのコントラスト」「明朝の字面の美しさ」を理由に **ヒラギノ明朝 ProN W3** を採用していた。commit `199ac95` で Sans に統一。Sans 化で得られた利点（観察ベース）：
...
「字面の美しさ」を優先するか「全アプリの一貫性 + 実用上の可読性」を優先するかのトレードオフ判断で、後者を選んだ形。
