...
- 既存配布物の対応:
  | アプリ | Light | Dark | 実現方式 |
  |---|---|---|---|
  | Xcode | 13pt (`ClaudeDay.xccolortheme`) | 14pt (`ClaudeDayDark.xccolortheme`) | テーマファイル内に font-size を直接定義 (`config/nix-darwin/files/xcode/`) |
  | Ghostty | 13pt | 14pt | Hammerspoon hook 経由 (`hammerspoon/init.lua` が `~/.config/ghostty/config.local` を appearance に応じて書き換え) |
...
- Xcode 側で Light 13pt → Dark 14pt にした際の使用感が良かったため、ターミナル含めた他アプリにも横展開する
...
3. appearance 変更を検知したら `~/.config/ghostty/config.local` を `font-size = 13` (Light) / `font-size = 14` (Dark) だけ含む最小ファイルとして上書き
4. Hammerspoon 起動時 (cold start / `hs.reload()`) にも 1 度同期して cold start でも正しい値に揃える

`config.local` は **gitignored** (`.gitignore` 末尾に登録) の machine-local 動的ファイル。Hammerspoon が起動していない別 Mac でも `?` (optional include) のおかげで Ghostty config は壊れない (font-size は本体 config の fallback = 14pt にフォールバックする)。

### Ghostty 側の挙動上の制約

- Ghostty の font-size は **既存ウィンドウには live reload されない**（glyph atlas 再構築を避ける設計）。Hammerspoon が `config.local` を書き換えても、appearance 切替の **次に開く新ウィンドウから**新サイズが適用される
- 全 window を即時更新したい場合は Ghostty を再起動するしかない（tmux session group 方式なので session 復元は問題なし）
- color / palette / keybinding は live reload 対象なので、appearance 切替時に色は即座に追従する（font-size だけ遅れる挙動）
...
- 本体 config に `font-size` を残す場合は **Dark と同値 (14pt)** にして、override 未適用時に小さい側に倒れない設計にする (= 戸惑わせない方の挙動にフォールバック)
