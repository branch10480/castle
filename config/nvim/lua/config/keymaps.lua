...
-- 外部アプリケーション連携
-- MarkdownObserver-Forkで現在のファイルを開く
local function open_in_markdown_observer()
	local filepath = vim.fn.expand("%")
	if filepath == "" then
		vim.notify("MarkdownObserver-Forkで開くファイルがありません", vim.log.levels.WARN)
		return
	end
	...
	if vim.v.shell_error ~= 0 then
		vim.notify("MarkdownObserver-Forkを開けませんでした: " .. result, vim.log.levels.ERROR)
	else
		vim.notify(
			"MarkdownObserver-Forkで開きました: " .. vim.fn.fnamemodify(fullpath, ":t"),
			vim.log.levels.INFO
		)
	end
end

keymap("n", "<leader>m", open_in_markdown_observer, { desc = "MarkdownObserver-Forkで開く" })
