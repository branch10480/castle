...
`config.local` は **gitignored** (`.gitignore` 末尾に登録) の machine-local 動的ファイル。Hammerspoon が起動していない別 Mac でも `?` (optional include) のおかげで Ghostty config は壊れない (font-size は本体 config の fallback = 12.5pt にフォールバックする)。
...
- 本体 config に `font-size` を残す場合は **Dark と同値 (12.5pt)** にして、override 未適用時に小さい側に倒れない設計にする (= 戸惑わせない方の挙動にフォールバック)
