...
- 既存配布物の対応:
  ...
  | Kaleidoscope | plist で管理 | plist で管理 | 適用外 (`config/nix-darwin/files/kaleidoscope/` の highlight.js 互換 CSS ペア `Claude Day-light.css` / `Claude Day-dark.css` は syntax 色のみ責任を持つ。エディタ font-size は plist `KSTextScopeFontInfoUserDefaultsKey` が真実の源で CSS からは制御不能。OS appearance 追従はファイル名 suffix `-light` / `-dark` で成立) |
