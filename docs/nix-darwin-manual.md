...
| **要警戒** | `fzf` | Nix の checkPhase でテストがハングする事例あり |
| 対処済み | `direnv` | macOS で checkPhase ハング → `doCheck=false` で回避（実例あり） |
...
3. `exec zsh -l` で新シェル起動 → hook 関数が Nix 版のパスで再生成される
...
| `_direnv_hook:2: no such file or directory: ...` | hook が古い brew パスを参照 | §4.4: `brew uninstall <tool>` → `exec zsh -l` |
