...
- **font-size も Light/Dark に追従** (Light 13pt / Dark 14pt) — Hammerspoon が appearance 変更を検知して `~/.config/ghostty/config.local` を書換 + Reload Configuration を自動発火する hook 方式。詳細は [`docs/font-strategy.md`](docs/font-strategy.md) §3.1 / [CLAUDE.md「テーマ運用ルール」](CLAUDE.md)
...
| Xcode | `config/nix-darwin/files/xcode/ClaudeDay.xccolortheme` / `ClaudeDayDark.xccolortheme`（Dark は Light より一律 +1pt） |
| Ghostty | `config/ghostty/themes/Claude Day` / `Claude Day Dark`（font-size の 1pt 差は Hammerspoon hook で実現、テーマファイルは color のみ） |
