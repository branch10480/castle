...
| `op-status` が `1Password CLI is locked` を返す。`op-status` は失敗するが `op account list` は通る（アカウントは見えるが session が無い状態） | GUI integration トグルが OFF に戻った / GUI と `op` daemon の握手が切れた（OS / 1Password アプリのアップデート後に起きやすい） | **初手で 1Password GUI → Settings (`⌘,`) → Developer → "Integrate with 1Password CLI" を目視確認**（OFF だったら ON。既に ON なら一度 OFF→ON にトグル）。それでもダメなら復旧手順は [`work-mac-setup.md` のトラブルシュート](work-mac-setup.md) の "1Password CLI integration の復旧" 節を参照 |
