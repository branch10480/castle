...
- **font-size も Light/Dark に追従** (Light 13pt / Dark 13.5pt) — Hammerspoon が appearance 変更を検知して `~/.config/ghostty/config.local` を書換 + Reload Configuration を自動発火する hook 方式。詳細は [`docs/font-strategy.md`](docs/font-strategy.md) §3.1 / [CLAUDE.md「テーマ運用ルール」](CLAUDE.md)
...
- **Hammerspoon** — `Ctrl+Space` でターミナル (Ghostty) にフォーカス / OS appearance 変更を watch して Ghostty の font-size を 0.5pt 差で自動切替 + Reload Configuration を発火（[`docs/font-strategy.md`](docs/font-strategy.md) §3.1）
...
| Xcode | `config/nix-darwin/files/xcode/ClaudeDay.xccolortheme` / `ClaudeDayDark.xccolortheme`（Dark は Light より一律 +0.5pt） |
| Ghostty | `config/ghostty/themes/Claude Day` / `Claude Day Dark`（font-size の 0.5pt 差は Hammerspoon hook で実現、テーマファイルは color のみ） |
