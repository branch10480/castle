...
| Touch ID プロンプトが毎回出る | (a) 1Password GUI が lock / GUI 連携 OFF (b) tmux ペインごとに `op run` を呼ぶ構成で per-pty 認可が走っている (= castle の MCP 用途で踏みやすい) | (a) は GUI を起動して unlock、Settings → Developer → "Integrate with 1Password CLI" を ON、auto-lock を緩める。(b) は別問題で 1Password の per-terminal-session 認可モデルに起因 → ghostty 起動時に env-file を pre-resolve して `op run` から `op://` を抜く A.2 ハイブリッド対策、詳細は [`op-touchid-investigation.md`](op-touchid-investigation.md) |
...
| `op-status` が `1Password CLI is locked` を返す（`op account list` は通る場合が多い） | session token が 30 分非アクティブで失効した（公式仕様、最頻パターン）/ GUI integration の Security / Developer トグルのどちらかが OFF / GUI と CLI の XPC 接続が切れた | **最初に `op signin` を試す**（idempotent。session が無ければ Touch ID で再生成、あれば何も起こらない）。それでも `locked` のままなら GUI 側 Settings → Security の Touch ID + Developer の Integrate トグルを目視確認。詳細復旧手順は [`work-mac-setup.md` のトラブルシュート](work-mac-setup.md) の "1Password CLI integration の復旧" 節を参照 |
