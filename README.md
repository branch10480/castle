...
- **font-size も Light/Dark に追従** (Light 12pt / Dark 13pt、+1pt 差) — Hammerspoon が appearance 変更を検知して `~/.config/ghostty/config.local` を書換 + Reload Configuration を自動発火する hook 方式。詳細は [`docs/font-strategy.md`](docs/font-strategy.md) §3.1 / [CLAUDE.md「テーマ運用ルール」](CLAUDE.md)
...
- Ghostty 互換キーで運用: ペイン分割 `Ctrl+;` / `Ctrl+'`、移動 `Ctrl+h/j/k/l`（vim-tmux-navigator 経由で nvim と seamless）、リサイズ `Ctrl+Shift+h/j/k/l`（横 3 セル / 縦 2 行で視覚的に揃える非対称 step）、均等化 `Ctrl+Shift+=`、copy mode `Ctrl+Shift+x`、ペイン破棄 `Ctrl+x`（即閉、shell の `C-x C-e` を犠牲にしてスピード優先）
...
| tmux 複数ペインでの per-pane Touch ID 回避 | ghostty 起動時に `op-warm-mcp` で `op://` を `/tmp/op-mcp-*.env` に literal 解決 → MCP の `op run` は素通し | [`docs/op-touchid-investigation.md`](docs/op-touchid-investigation.md) |
...
- **Hammerspoon** — `Ctrl+Space` でターミナル (Ghostty) にフォーカス / OS appearance 変更を watch して Ghostty の font-size を 1pt 差（Light 12pt / Dark 13pt）で自動切替 + Reload Configuration を発火（[`docs/font-strategy.md`](docs/font-strategy.md) §3.1）
...
- 主要スキル:
  ...
  - **`/htmla`** — Claude Day デザイン言語の HTML 成果物（spec / report / mockup / プロトタイプ等）を生成。使い方: [`docs/htmla-usage.html`](docs/htmla-usage.html)
  - **`/zama-parking`** — イオンモール座間 駐車場空き状況確認
- セッション横断検索 (zsh 関数, `home/.zshrc.d/clog.zsh`):
  - **`clog <query>...`** — `~/.claude/projects/**/*.jsonl` を全 working directory 横断で grep（「あの話をしたのはどの worktree だったっけ」を解決）。`-g` で cwd 集計、`--since 7d` で期間絞り、`--cwd <part>` で cwd 絞り、`--role user` で発話者絞り
...
| サーフェス | 担当ファイル |
|---|---|
| HTML 成果物（`/htmla` 出力） | `claude/skills/htmla/design-system.html` |
| MarkdownObserver | `config/nix-darwin/files/markdownobserver/user.css` |
| Xcode | `config/nix-darwin/files/xcode/ClaudeDay.xccolortheme` / `ClaudeDayDark.xccolortheme`（Light 11.5pt / Dark 12.5pt、Dark は +1pt） |
| Ghostty + cmux | `config/ghostty/themes/Claude Day` / `Claude Day Dark`（font-size の 1pt 差は Hammerspoon hook で実現、テーマファイルは color のみ。cmux は libghostty 内蔵で同 config を共有） |
...
| [`docs/op-touchid-investigation.md`](docs/op-touchid-investigation.md) | tmux 複数ペイン → claude → MCP で Touch ID が連発した件の根本原因と A.2 ハイブリッド対策 |
...
| [`docs/htmla-usage.html`](docs/htmla-usage.html) | `/htmla` スキルの使い方ガイド |
