...
-- ─────────────────────────────────────────────────────────────
-- Ghostty appearance (font-size + Liquid Glass) を OS に追従させる
-- ─────────────────────────────────────────────────────────────
-- castle のテーマ運用ルールを Ghostty で実現する:
--   - font-size: Light=12 / Dark=13 (「Light vs Dark = +1pt」契約)
--   - background-blur: Light=macos-glass-regular / Dark=macos-glass-clear
--     (macOS 26 Tahoe Liquid Glass material を appearance で使い分け)
--
-- 仕組み:
...
--   blur 値も同じ config.local に乗せる (両方とも reload で反映できる)。
--
-- 動作:
...
--   3. ~/.config/ghostty/config.local を `font-size = N` + `background-blur = ...`
--      の最小ファイルとして上書き
--   4. Ghostty メニュー → Reload Configuration を hs.application 経由で
--      自動発火 (Ghostty 起動中のみ best-effort)。Ghostty は reload で
--      color / palette / font-size / background-blur を含む全設定を
--      **既存ウィンドウにも即時反映** する (実機確認済み、2026-05)
--   5. Hammerspoon 起動時にも 1 度同期 (cold start で正しい値に揃える)
--
-- なぜ background-opacity を hook で切り替えないか:
--   Ghostty 仕様で「On macOS, changing background-opacity requires restarting
--   Ghostty completely」と man page で明記されている (AppKit の
--   NSWindow.isOpaque が window 初期化時固定属性のため)。reload で反映
--   されないので、opacity は本体 config 側で 0.9 に固定する。blur 値は
--   restart 必須の記載が無く、reload で反映可能。
--
...
local FONT_SIZE_DARK = 13
-- macOS 26 Tahoe Liquid Glass material の Light/Dark 切替。
-- - macos-glass-regular: やや不透明な glass (Light で文字が浮きにくい)
-- - macos-glass-clear:   高透過な glass (Dark で背景の warm gray が透ける)
-- どちらも native NSVisualEffectView material のため OS appearance 内部の
-- 明度自動補正も乗る (= Hammerspoon hook が値を切り替えると、material 側の
-- 自動追従と合わせて二段階で印象が変わる、という意図)。
local BG_BLUR_LIGHT = "macos-glass-regular"
local BG_BLUR_DARK = "macos-glass-clear"
...
local function syncGhosttyAppearanceConfig()
    local style = currentInterfaceStyle()
    local size = (style == "Dark") and FONT_SIZE_DARK or FONT_SIZE_LIGHT
    local blur = (style == "Dark") and BG_BLUR_DARK or BG_BLUR_LIGHT
    local f = io.open(GHOSTTY_LOCAL_CONFIG, "w")
    if not f then return end
    f:write("# auto-generated by hammerspoon/init.lua — do not edit manually\n")
    -- %g を使うことで 13 は "13"、12.5 は "12.5" と必要桁数だけ出力される。
    -- %d は浮動小数を渡すと "bad argument" になるため、将来 0.5pt 刻みの
    -- 中間値に戻す可能性も含めて %g で安全側に倒す (現行は整数ペア 12/13)。
    f:write(string.format("font-size = %g\n", size))
    f:write(string.format("background-blur = %s\n", blur))
    f:close()
    reloadGhosttyConfig()
end

-- Hammerspoon 起動 (= cold start / hs.reload()) 時に 1 度同期
syncGhosttyAppearanceConfig()
